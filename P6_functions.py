...
def overview_fig(title, df, col):
    """
    Method used to plot overview of a 

    Parameters:
    -----------------
        title : title of the global plot
        df (pandas.DataFrame): Dataset to plot
        col: column of dataset to explore
        
    Returns:
    -----------------
        None. 
        PLot the overview of the column of the dataset selectionned. 
    """
    fig = plt.figure(constrained_layout=True, figsize=[13,5])
    fig.suptitle(title, size=25, fontweight="bold", y=1.5) #, y=1.05)
    spec = fig.add_gridspec(nrows=2, ncols=2, width_ratios=[1, 1], height_ratios=[5, 1])
    
    ax_box = fig.add_subplot(spec[1, 0])
    ax_hist = fig.add_subplot(spec[0, 0])
    ax_qqplot = fig.add_subplot(spec[:, 1])

    # Histplot
    sns.histplot(data=df, x=df[col], bins=75,  kde=True,  ax=ax_hist)
    ax_hist.set(xlabel="", xticks=[])

    # Boxplot
    boxplot = sns.boxplot(data=df, x=df[col], showmeans=True, ax=ax_box,
                          meanprops={"markerfacecolor":"white", "markeredgecolor":"blue"})
    ax_box.set(xlabel=title) #, xticks=[])
    # ax_box.set(yticks=[])
    ax_box.set_ylabel("Distribution", rotation=0)
    boxplot.set_title("")

    
    # QQplot
    qqplot(df[col], line='s', ax=ax_qqplot)
    
    fig.align_ylabels()
    
    # plt.savefig(img_out)
    name = title.replace(' ','-')
    plt.savefig("images/qqplot-of-" + name + ".png")
    plt.show()
