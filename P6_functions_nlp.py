...
def reduce_incorrect_character_repeatition(text):
    """
...
def tokenizer(text, type='spacy'):
    ...
    -----------------
        ...
        type (str): package used ["nltk", "spacy"]
    ...
    if type == 'nltk':
        ...
        raise ValueError("Unsupported type for tokenizer function on functions_nlp.py")
...
def cleaning_up_text(text, contractions, type='spacy'):
    """
    ...
    text = reduce_incorrect_character_repeatition(text)
    text = lowercase_words(text)
    text = expand_contractions(text, contractions)
    text = remove_punctuation(text)
    text = remove_non_alphabetic(text)
    words = tokenizer(text, type)

    return words


def remove_stop_words(words, language="english", type='spacy'):
    """
    ...
    if type == 'nltk':
        ...
        raise ValueError("Unsupported type for remove_stop_words function on functions_nlp.py")
...
def remove_non_english_words(words, type='spacy'):
    """
    ...
    if type == 'nltk':
        ...
        # Create a string to pass to nlp function
        text = ' '.join(words)
        
        # Create a Doc Object Spacy
        doc= nlp(text)
        
        filtered_words = [token.text for token in doc if not token.is_oov]

    return filtered_words


def keep_nouns(words, type='spacy'):
    """
    ...
    if type == 'nltk':
        ...
        filtered_words = [token.text for token in doc if (token.tag_ == "NN" or token.tag_ == "NNP" or
                      token.tag_ == "NNPS" or token.tag_ == "NNS")]        

    return filtered_words


def remove_words(words, language, type='spacy'):
    """
    ...
    words = remove_stop_words(words, language, type)
    words = remove_non_english_words(words, type)
    words = keep_nouns(words, type)
